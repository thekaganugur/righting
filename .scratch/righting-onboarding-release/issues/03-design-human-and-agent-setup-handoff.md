...
2. **Manual maintainer:** published package onboarding and reference documentation describe the same policy-decision and approval process plus the equivalent additive ESLint integration. It is a first-class route; a maintainer is never required to install or read agent skills. Re-running `righting init` safely confirms that Righting recognizes the completed policy while a read-only inspection surface remains future work.
