...
- Generated source uses the `.generated.` marker or `generated/` directory segment. Generated status tells coding agents not to edit the file, but does not exempt it: the file must also resolve to a canonical role and follows that role's dependency rules.
- A canonical `composition-root` filename identifies a visible non-role wiring file. It may depend on role entry points, governed role files cannot depend on it, and project convention tokens may add names such as `main` or `bootstrap` without creating a role.
