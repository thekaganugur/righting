...
- an approved complete `righting.json` whose applicable static capabilities do not include `protected-dependency` or `context-firewall`;
...
Preserve the project's existing Oxlint plugins, categories, rules, and command. Run that command from the project root. Adapter loading invokes the adjacent packaged CLI as `righting inspect --json`, requires inspection schema and normalized contract version 1, and fails before linting when inspection is unsuccessful, the policy is incomplete, a version is unsupported, or an unsupported static capability applies. It neither imports `righting/core` nor reads or normalizes `righting.json` itself.
...
Unsupported capabilities: `protected-dependency`, `context-firewall`, and guidance-only `design-judgment`. Contracts where either unsupported static capability applies are rejected at plugin load rather than partially enforced. Files outside declared coverage and runtime dependency behavior remain outside static evidence.
