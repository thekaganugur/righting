...
The project covers `src/**/*.ts`. Its approved directory-token aliases are `Screen` (Client), `Workflow` (Manager), `Rules` (Engine), `Gateway` (ResourceAccess), `Api` (Resource), and `Shared` (Utility); `main` is an approved composition-root token. It has no role-edge overrides. The exact choices live in its `righting.json`; they are evidence for this project only.
...
3. A new Orders refund workflow initially imported another Manager directly. The normal lint loop reported `righting/role-dependency`.
