...
Done when every core use case is named, every volatility is stated via an axis and tagged Observed / Projected / Speculative, every architecture-relevant source file used as evidence or included in a candidate has a role hypothesis or explicit non-role treatment, and accepted Module ownership or its absence is explicit. Surface candidates only after these anchors hold.
...
The reference owns the report structure, candidate fields, diagrams, styling, and Top recommendation. A Speculative candidate may be listed but cannot be the top recommendation.
