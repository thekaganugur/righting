...
For each result, report the recommendation, evidence, and material pros and cons. When deeper design would help, recommend `righting-bounded-contexts` with its pros and cons, then ask permission before invoking it. Invoke it only after approval, pass it the gathered evidence, and resume this integration from its fixed-heading result; do not treat specialist approval as approval of the complete `righting.json`. When maintainer input is needed, ask one question at a time and keep it short.
